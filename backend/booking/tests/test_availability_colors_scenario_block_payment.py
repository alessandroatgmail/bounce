"""
Same end-to-end scenario as test_availability_colors_scenario.py, but the
festival has block_payment=True: every booking that would normally land
ACCEPTED lands APPROVING instead. APPROVING must count toward capacity and
role balance exactly like ACCEPTED does, so the waiting-list thresholds and
children_levels colors should play out identically — only the status label
differs.

Fixture: one multi_events, non-free, block_payment festival with 4 children
all at a single "Beginner" level, capacity 10, extras 2, warning_threshold 3,
two partner roles (Leader, Follower).
"""
import pytest
...
def mini_festival(admin_client, world_data):
    """Same as test_availability_colors_scenario.mini_festival, but the
    festival is created with block_payment=True."""
...
class TestAvailabilityColorsScenarioBlockPayment:
    ...
    def test_three_leaders_then_a_follower(self, mini_festival, admin_client):
        ...
        # ── 1st leader: approving, still green ─────────────────────────
        ...
        assert contribution1.status == ContributionStatus.APPROVING
        colors = level_colors(admin_client, mini_festival)
        assert colors["Leader"] == "green"

        # ── 2nd leader: approving, but now orange (extras=2 boundary) ──
        ...
        assert contribution2.status == ContributionStatus.APPROVING
        colors = level_colors(admin_client, mini_festival)
        ...
        # ── 1st follower: approving (minority role) ──────────────────────
        ...
        assert contribution4.status == ContributionStatus.APPROVING

        contribution3.refresh_from_db()
        colors = level_colors(admin_client, mini_festival)
        assert contribution3.status == ContributionStatus.APPROVING
        ...
        # ── 5th leader, 2nd follower: Both Approving (6 approving, 1 waiting) ──
        ...
        assert contribution6.status == ContributionStatus.APPROVING
        assert contribution7.status == ContributionStatus.APPROVING
        contribution5.refresh_from_db()
        assert contribution5.status == ContributionStatus.WAITING
        colors = level_colors(admin_client, mini_festival)
        assert colors["Leader"] == "orange"

        # ── 6th leader, 3rd follower: Both Approving (8 approving, 1 waiting) ──
        ...
        assert contribution8.status == ContributionStatus.APPROVING
        assert contribution9.status == ContributionStatus.APPROVING
        contribution5.refresh_from_db()
        assert contribution5.status == ContributionStatus.WAITING
        colors = level_colors(admin_client, mini_festival)
        assert colors["Leader"] == "orange"
        ...
        # ── 7th leader, 4th follower: Both Approving (11 approving, 1 waiting) ──
        ...
        assert contribution10.status == ContributionStatus.APPROVING
        assert contribution11.status == ContributionStatus.APPROVING
        contribution5.refresh_from_db()
        assert contribution5.status == ContributionStatus.WAITING
        colors = level_colors(admin_client, mini_festival)
        ...
        # ── 5th follower: now goes in waiting list, full (11 approving, 2 waiting) ──
        ...
        # ── 1st follower cancels: 5th follower in waiting list should be
        #    promoted — to APPROVING (not ACCEPTED), since the festival
        #    blocks payment ──────────────────────────────────────────────
        ...
        assert contribution12.status == ContributionStatus.APPROVING
