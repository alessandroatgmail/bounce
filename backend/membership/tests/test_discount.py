...
class TestDiscountStudent:
    ...
    def test_unauthenticated_list_returns_403(self, student_client):
        response = student_client.get(LIST_URL)
        assert response.status_code == http_status.HTTP_403_FORBIDDEN

    def test_unauthenticated_retrieve_returns_403(self, student_client, db):
        discount = Discount.objects.first()
    ...
    def test_unauthenticated_update_returns_403(self, student_client, db):
        discount = Discount.objects.first()
        response = student_client.put(detail_url(discount.pk), create_discount_payload(), format="json")
        assert response.status_code == http_status.HTTP_403_FORBIDDEN

    def test_unauthenticated_delete_returns_403(self, student_client, db):
        discount = Discount.objects.first()
...
class TestDiscountStaff:
    ...
    def test_staff_list_returns_200(self, staff_client):
        response = staff_client.get(LIST_URL)
        assert response.status_code == http_status.HTTP_200_OK
        assert len(response.data) == 3

    def test_staff_retrieve_returns_200(self, staff_client, db):
        discount = Discount.objects.first()
    ...
    def test_update_returns_204(self, staff_client, db):
        discount = Discount.objects.first()
        response = staff_client.put(detail_url(discount.pk), create_discount_payload(), format="json")
        assert response.status_code == http_status.HTTP_200_OK

    def test_delete_returns_204(self, staff_client, db):
        discount = Discount.objects.first()
